...
type Props = {
  open: boolean;
  conflicts: Conflict[];
  // Optional backend-provided error text (e.g., availability message)
...
export default function ConflictModal({ open, conflicts, errorMessage, onReplace, onCancel, onClose }: Props) {
  const hasTimeConflicts = Array.isArray(conflicts) && conflicts.length > 0;
  return (
    <Dialog 
      open={open} 
      onClose={onClose}
    ...
    >
      <DialogTitle>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          {hasTimeConflicts ? 'Assignment Conflict' : 'Cannot Complete Action'}
        </Box>
      </DialogTitle>
      <DialogContent>
        {hasTimeConflicts ? (
          ...
          <>
            <Alert severity="warning" sx={{ mb: 2 }}>
              {errorMessage || 'This change cannot be completed due to availability or schedule rules.'}
            </Alert>
            <Typography variant="body2" color="text.secondary">
              There are no overlapping assignments to replace. Adjust the time or pick a different aide.
            </Typography>
          </>
        )}
      </DialogContent>
